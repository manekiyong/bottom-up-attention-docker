...
from utils.extract_utils import get_image_blob, save_bbox, save_roi_features_by_bbox, save_roi_features, save_fast_rcnn_features
...
def extract_feat_singlegpu(split_idx, img_list, cfg, args):  
    ...
    for im_file in tqdm(img_list):
        ...
        if args.extract_mode == 'roi_feats': # cfg.MODEL.BUA.EXTRACTOR.MODE == 1:
            attr_scores = None
        ...
        # extract bbox only
        elif args.extract_mode == 'bboxes': # cfg.MODEL.BUA.EXTRACTOR.MODE == 2:
            with torch.set_grad_enabled(False):
        ...
        # extract roi features by bbox
        elif args.extract_mode == 'bbox_feats': # cfg.MODEL.BUA.EXTRACTOR.MODE == 3
            ...
            generate_npz(3, 
                ...
                boxes, scores, features_pooled, attr_scores)
                # extract roi features
        elif args.extract_mode == 'fast_rcnn':
            attr_scores = None
            with torch.set_grad_enabled(False):
                if cfg.MODEL.BUA.ATTRIBUTE_ON:
                    boxes, scores, features_pooled, attr_scores = model([dataset_dict])  # caffe mode
