...
def save_fast_rcnn_features(args, cfg, im_file, im, dataset_dict, boxes, scores, features_pooled, attr_scores=None):
    MIN_BOXES = cfg.MODEL.BUA.EXTRACTOR.MIN_BOXES
    MAX_BOXES = cfg.MODEL.BUA.EXTRACTOR.MAX_BOXES
    CONF_THRESH = cfg.MODEL.BUA.EXTRACTOR.CONF_THRESH
    ...
    image_feat = feats[keep_boxes]
    image_bboxes = dets[keep_boxes]
    cls_prob = scores[keep_boxes].numpy()
    attr_scores = attr_scores[0]
    attr_prob = attr_scores[keep_boxes].numpy()

    #
    output_file = os.path.join(args.output_dir, im_file.split('.')[0])
    np.savez_compressed(output_file, x=image_feat, bbox=image_bboxes, num_bbox=len(keep_boxes), image_h=np.size(im, 0), image_w=np.size(im, 1), cls_prob=cls_prob, attr_prob=attr_prob)
